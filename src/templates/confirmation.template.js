...
<mj-body background-color="#F2F2F2">
  ...
  <mj-section background-color="#ffffff" border="1px solid #e9e9e9" padding-top="60px" padding-bottom="60px">
    <mj-column width="400px">
      ...
      <mj-button background-color="#0060e8" align="left" href="{{accountConfirmationUrl}}">Confirm account</mj-button>
 ...
 * @returns {html}
 */
function confirmationTemplate(context) {
  const template = compile(confirmationMJML);
  const mjml = template(context);
  const emailOutput = mjml2html(mjml);
  return emailOutput.html;
}
